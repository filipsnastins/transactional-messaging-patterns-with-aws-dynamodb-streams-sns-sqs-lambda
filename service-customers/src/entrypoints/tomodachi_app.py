...
class TomodachiService(tomodachi.Service):
    ...
    @tomodachi.aws_sns_sqs("order--created", queue_name="customer--order-created", message_envelope=JsonBase)
    async def order_created_handler(self, data: dict) -> None:
        async with DynamoDBUnitOfWork(message_id=data["event_id"]) as uow:
    ...
    @tomodachi.aws_sns_sqs("order--cancelled", queue_name="customer--order-cancelled", message_envelope=JsonBase)
